...
void ResetSelectedVars(BGPQuery *query, const vector<string> &_query_var) {
	query->selected_var_id.clear();
	query->selected_so_var_num = 0;
	query->selected_pre_var_num = 0;
	query->total_selected_var_num = 0;

	for (const auto &var_name : _query_var) {
		auto id_it = query->var_item_to_id.find(var_name);
		if (id_it == query->var_item_to_id.end()) continue;
		unsigned var_id = id_it->second;
		query->selected_var_id.push_back(var_id);
		if (query->get_vardescrip_by_id(var_id)->var_type_ == VarDescriptor::VarType::Predicate)
			query->selected_pre_var_num += 1;
		else
			query->selected_so_var_num += 1;
	}
	query->total_selected_var_num = query->selected_var_id.size();
}
...
void BGPQuery::ScanAllVar(const vector<string>& _query_var) {
	...
	ResetSelectedVars(this, _query_var);
}
...
bool BGPQuery::is_var_satellite_by_index(unsigned index) {
	...
	return (var_descrip->degree_ == 1 && var_descrip->selected_ == false);
}
