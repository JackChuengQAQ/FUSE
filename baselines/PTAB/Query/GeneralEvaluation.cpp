...
GeneralEvaluation::GeneralEvaluation(KVstore *_kvstore, Statistics *_statistics, StringIndex *_stringindex, QueryCache *_query_cache, \
	...
	TYPE_ENTITY_LITERAL_ID _limitID_entity, CSR *_csr, HopIndex *hop_index, shared_ptr<Transaction> _txn):
	...
	limitID_entity(_limitID_entity), temp_result(NULL), fp(NULL), export_flag(false), csr(_csr),hop_index_(hop_index), txn(_txn)
{
	if (csr)
...
GeneralEvaluation::loadCSR()
{
	...
	for(unsigned i = 0; i < pre_num; i++)
	{
		...
		cout<<csr[0].offset_list[i].size()<<endl;	
		cout<<csr[0].adjacency_list[i].size()<<endl;	
	}
...
bool GeneralEvaluation::doQuery(TopKStrategy top_k_method)
{
	query_tree.print();
    ...
    this->optimizer_ = make_shared<Optimizer>(kvstore,statistics,pre2num,pre2sub,pre2obj,limitID_predicate,
                                      limitID_literal,limitID_entity,hop_index_,txn);
	...
	this->temp_result = this->queryEvaluation(0,top_k_method);
...
TempResultSet* GeneralEvaluation::queryEvaluation(int dep,TopKStrategy top_k_method)
{
				...
				(group_pattern.sub_group_pattern[i + 1].type != QueryTree::GroupPattern::SubGroupPattern::Pattern_type && group_pattern.sub_group_pattern[i + 1].type != QueryTree::GroupPattern::SubGroupPattern::Union_type))
			{
				...
				for (int j = st; j <= i; j++)
					if (group_pattern.sub_group_pattern[j].type == QueryTree::GroupPattern::SubGroupPattern::Pattern_type)
					{
						...
						{
							if (group_pattern.getRootPatternBlockID(j) == j)		
							{
								Varset occur;
		...
		else if (group_pattern.sub_group_pattern[i].type == QueryTree::GroupPattern::SubGroupPattern::Union_type)
		{
			...
			for (int j = 0; j < (int)group_pattern.sub_group_pattern[i].unions.size(); j++)
			{
				...
				{			
					...
					this->optimizer_->DoQuery(sparql_query,query_info);
					#else
					this->optimizer_->DoQuery(bgp_query,query_info,top_k_method);
					#endif
...
#ifndef TEST_BGPQUERY
					int basic_query_num = sparql_query.getBasicQueryNum();
#else
					int basic_query_num = encode_varset.empty() ? 0 : 1;
#endif
					for (int j = 0; j < basic_query_num; j++)
					{
						TempResultSet *temp = new TempResultSet();
						temp->results.push_back(TempResult());
	...
	if (this->query_tree.getQueryForm() == QueryTree::Select_Query)
	{
		...
		if (this->query_tree.checkAtLeastOneAggregateFunction() || !this->query_tree.getGroupByVarset().empty())
		{
				...
				proj[0].aggregate_type != QueryTree::ProjectionVar::Max_type)
			{
				...
				bool exist = 0, earlyBreak = 0;	
				stringstream ss;
				bool notFirstOutput = 0;	
				ss << "\"{\"paths\":[";
				for (int uid : uid_ls)
				{
					for (int vid : vid_ls)
