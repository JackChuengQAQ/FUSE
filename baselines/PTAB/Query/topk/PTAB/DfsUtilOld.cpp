...
DfsFRIterator *DfsUtilDynamic::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan,
  ...
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID, 
                       std::pair<std::shared_ptr<DfsOWIterator>, 
                                 NodeOneChildVarPredicatesPtr>>> 
  descendents_OWs;
  ...
  for (unsigned descendent_i = 0; descendent_i < descendents_num; descendent_i++) {
    ...
    if (is_fr) {
      ...
      std::map<TYPE_ENTITY_LITERAL_ID, 
               std::pair<std::shared_ptr<DfsOWIterator>, 
                         NodeOneChildVarPredicatesPtr>> 
...
std::map<TYPE_ENTITY_LITERAL_ID, std::shared_ptr<OldFQIterator>>
DfsUtilDynamic::AssemblingFrOw(std::set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                               int fq_var_id, int k,
  ...
  unsigned child_type_num = ow_size + fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID, std::shared_ptr<OldFQIterator>> id_fqs;
  for (auto fq_id : fq_ids) {
    double score = 0.0;
    if (node_scores != nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<OldFQIterator>(k, fq_id, fq_var_id, child_type_num, score);
    
    for (unsigned i = 0; i < ow_size; i++) {
...
std::shared_ptr<OldFQIterator>
DfsUtilDynamic::AssemblingTheFq(TYPE_ENTITY_LITERAL_ID fq_id,
                                ...
                                std::vector<std::pair<std::shared_ptr<DfsOWIterator>, NodeOneChildVarPredicatesPtr>> &descendents_OWs)
{
  ...
  auto fq = make_shared<OldFQIterator>(k, fq_id, fq_var_id, child_type_num, node_score);
...
std::pair<std::shared_ptr<DfsOWIterator>, 
          NodeOneChildVarPredicatesPtr> 
DfsUtilDynamic::ExploreOW(TYPE_ENTITY_LITERAL_ID parent_id,
                          int child_var,
  ...
  auto valid_pre_to_num = hop_index->IsValidPredicateToNumber(pre_id);
  if (has_coefficient && tree_edges.predicate_ids_.size() == 1 && pre_constant &&
    ...
    for (unsigned int range_i = 1; range_i < D_HOP_LOG; range_i++) {
      RangeState range_state = hop_index->mask_[value_startD + range_i];
      if (range_state == RangeState::NoExist)
        distinct_level = 1 << range_i; 
      else if (range_state == RangeState::UnExact && uncertain_level == 0)
        uncertain_level = 1 << range_i; 
    }
  ...
  std::for_each(child_candidates->cBegin(), child_candidates->cEnd(),
                [&children_ids_set](const std::pair<const TYPE_ENTITY_LITERAL_ID, 
...
DfsUtilDynamic::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge> &tree_edges_,
  ...
  std::set<TYPE_ENTITY_LITERAL_ID> child_candidates = TopKUtil::ExtendTreeEdge(parent_var_candidates, child_var,
                                                                              ...
                                                                              child_parent, tree_edges_, dfs_help_info->env);
...
shared_ptr<OldFQIterator> DfsUtilDynamic::ExploreFQ(TYPE_ENTITY_LITERAL_ID chosen_child,
                                                    int child_var,
                                                    size_t &distinct_level,
                                                    size_t &uncertain_level,
                                                    DfsHelpInfo *dfs_help_info) {
  shared_ptr<OldFQIterator> fq(nullptr);
  ...
  if (cache_it != node_caches->var_valid_ids[child_var].end())
    return std::dynamic_pointer_cast<OldFQIterator>(cache_it->second);
...
DfsUtilDynamic::ExploreFRs(int parent_var,
                           int parent_id,
                           int child_var,
                           size_t &distinct_level,
                           size_t &uncertain_level,
  ...
  shared_ptr<IDListWithAppending> children_predicates = TopKUtil::DfsExtendTreeEdge(parent_id, child_var,
  ...
  while (!success_found && fq_it != children_predicates->End()) {
    distinct_level = 0;
    uncertain_level = 0;
    
    chosen_child = fq_it->first;
    fq = ExploreFQ(chosen_child, child_var, distinct_level, uncertain_level, dfs_help_info);
    if (fq == nullptr)
  ...
  if (success_found && distinct_level > 0) {
  ...
  r->Insert(chosen_child, fq, children_predicates->cBegin()->second);
  children_predicates->Erase(chosen_child);

  
  if (uncertain_level & 0x1) {
    auto fq_it2 = children_predicates->Begin();
    while (fq_it2 != children_predicates->End()) {
      distinct_level = 0;
      uncertain_level = 0;
      
      auto child_id = fq_it2->first;
      auto fq2 = ExploreFQ(child_id, child_var, distinct_level, uncertain_level, dfs_help_info);
      if (fq2 != nullptr)
        r->Insert(child_id, fq2, fq_it2->second);
      fq_it2 = children_predicates->Erase(fq_it2);
    }
  }
