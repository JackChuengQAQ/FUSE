...
void OldFQIterator::TryGetNext(DfsHelpInfo *dfs_help_info)
{
  ...
  while(!this->queue_.empty())
  {
    ...
    else if(type_value == DfsValue::Exact)
    {
      ...
      throw string("wrong DfsValue in OldFQIterator::TryGetNext");
  }
}

void OldFQIterator::Insert(std::shared_ptr<DfsList> FR_OW_iterator) {
...
void OldFQIterator::Insert(std::vector<std::shared_ptr<DfsList>> FR_OW_iterators) {
  this->fr_ow_iterators_ = std::move(FR_OW_iterators);
}

void OldFQIterator::GetResult(int i_th, std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
