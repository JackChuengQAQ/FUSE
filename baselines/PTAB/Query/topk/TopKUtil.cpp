...
namespace {
std::shared_ptr<EmbeddingGraph::ScoreContext> g_embedding_score_context;
std::vector<std::string> g_ground_truth_output_lines;
}

void TopKUtil::SetEmbeddingScoreContext(std::shared_ptr<EmbeddingGraph::ScoreContext> context) {
  g_embedding_score_context = context;
}

std::shared_ptr<EmbeddingGraph::ScoreContext> TopKUtil::GetEmbeddingScoreContext() {
  return g_embedding_score_context;
}

void TopKUtil::ClearGroundTruthOutputLines() {
  g_ground_truth_output_lines.clear();
}

void TopKUtil::AddGroundTruthOutputLine(const std::string &line) {
  g_ground_truth_output_lines.push_back(line);
}

bool TopKUtil::HasGroundTruthOutputLines() {
  return !g_ground_truth_output_lines.empty();
}

bool TopKUtil::WriteGroundTruthOutput(FILE *fp) {
  if (fp == nullptr) return false;
  for (const auto &line : g_ground_truth_output_lines) {
...
void TopKUtil::FillArrayOfTreeNode(TopKTreeNode *top_k_tree_node,
                                   TopKTreeNode **node_plan) {
    auto descendent_num = top_k_tree_node->descendents_.size();
    auto var_id = top_k_tree_node->var_id;
    node_plan[var_id] = top_k_tree_node;
    auto ow_num = top_k_tree_node->descendents_ow_num_;
    unsigned i = 0;
    for (; i < ow_num; i++) {
        auto ow_descendent = top_k_tree_node->descendents_[i];
        FillArrayOfTreeNode(ow_descendent, node_plan);
    }
    for (; i < descendent_num; i++) {
        auto fr_descendent = top_k_tree_node->descendents_[i];
        FillArrayOfTreeNode(fr_descendent, node_plan);
    }
}
                                          ...
                                          bool minus_signed)
{
      ...
      comp_tree_node->rchild->lchild==nullptr&&comp_tree_node->rchild->rchild==nullptr)
  {
    ...
    {
      auto &val_string = comp_tree_node->rchild->val; 
...
std::shared_ptr<std::unordered_map<std::string,double>> TopKUtil::getVarCoefficients(QueryTree::Order order)
{
#ifdef TOPK_DEBUG_INFO
  order.comp_tree_root->print(0);
#endif
  auto r= make_shared<std::unordered_map<std::string,double>>();
...
TopKUtil::getEmbeddingSumCoefficients(const shared_ptr<BGPQuery> &bgp_query) {
  auto r = make_shared<std::unordered_map<std::string, double>>();
  for (int i = 0; i < bgp_query->get_total_var_num(); ++i) {
    auto var_descriptor = bgp_query->get_vardescrip_by_index(i);
    if (var_descriptor->var_type_ == VarDescriptor::VarType::Predicate)
      continue;
    (*r)[bgp_query->get_var_name_by_id(i)] = 1.0;
  }
  return r;
}
...
TopKUtil::GetChildNodeScores(double coefficient,
  ...
  auto embedding_context = env == nullptr ? nullptr : env->embedding_score_context;
  if (embedding_context == nullptr)
    embedding_context = TopKUtil::GetEmbeddingScoreContext();
  bool use_embedding_score = embedding_context != nullptr && embedding_context->Enabled() &&
                             env != nullptr && env->bgp_query != nullptr && query_var_id >= 0;
  string query_var_name;
  if (use_embedding_score)
    query_var_name = env->bgp_query->get_var_name_by_id(query_var_id);

  for (unsigned child_id : child_candidates) {
    ...
    if (use_embedding_score) {
      if (!Util::is_entity_ele(child_id)) {
        ...
        string node_term = env->kv_store->getStringByID(child_id);
        delete_it = !embedding_context->GetScore(query_var_name, child_id, node_term, &similarity);
        if (!delete_it)
          (*node_score)[child_id] = coefficient * similarity;
      }
    ...
    if (!delete_it)
      (*node_score)[child_id] = coefficient * numeric_score;
    if (parent_child != nullptr && delete_it) {
      auto &its_parents = (*child_parent)[child_id];
      for (auto parent_id : its_parents) {
        auto children_and_predicates = (*parent_child)[parent_id];
...
std::set<TYPE_ENTITY_LITERAL_ID> 
TopKUtil::ExtendTreeEdge(std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates, int child_var,
  ...
  const auto &predicates_constant = tree_edges_->predicate_constant_;
  const auto &predicate_ids = tree_edges_->predicate_ids_;
  const auto &directions = tree_edges_->directions_;
...
TopKUtil::DfsExtendTreeEdge(TYPE_ENTITY_LITERAL_ID parent_id,int child_var,
  ...
  bool id_list_prepared = false;

  shared_ptr<IDListWithAppending> appending_list_ptr = nullptr;
  bool appending_list_prepared = false;

  unsigned two_var_edges_num = predicate_ids.size();
  unsigned i;
  for (i = 0; i < two_var_edges_num; i++) {
  ...
  if(child_id_caches_it!=env->id_caches->end())
  {
    ...
    auto child_id_end_it = child_id_caches->end();
    set<TYPE_ENTITY_LITERAL_ID> deleted_children;
    auto it = appending_list_ptr->contents_->cbegin();
    auto c_end = appending_list_ptr->contents_->cend();
    for (;it!=c_end;it++) {
    ...
    for(auto deleted_id : deleted_children)
      appending_list_ptr->Erase(deleted_id);
  }


  return appending_list_ptr;
}
