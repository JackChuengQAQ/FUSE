...
namespace {
thread_local std::vector<ui> g_intersection_visit_mark;
thread_local ui g_intersection_epoch = 1;

static ui nextIntersectionEpoch() {
    g_intersection_epoch += 1;
    if (g_intersection_epoch == 0) {
        std::fill(g_intersection_visit_mark.begin(), g_intersection_visit_mark.end(), 0);
        g_intersection_epoch = 1;
    }
    return g_intersection_epoch;
}

static void ensureIntersectionMark(ui data_vertices_num) {
    if (g_intersection_visit_mark.size() < data_vertices_num) {
        g_intersection_visit_mark.resize(data_vertices_num);
    }
}
}  
...
FilterVertices::LDFFilter(const Graph *data_graph, const Graph *query_graph, ui **&candidates, ui *&candidates_count) {
    ...
    const ui data_vertices_num = data_graph->getVerticesCount();
    if (VertexLabelCompat::isIgnoreVertexLabels()) {
        for (ui i = 0; i < query_graph->getVerticesCount(); ++i) {
            ui degree = query_graph->getVertexDegree(i);
            for (VertexID dv = 0; dv < data_vertices_num; ++dv) {
                if (data_graph->getVertexDegree(dv) >= degree) {
                    candidates[i][candidates_count[i]++] = dv;
                }
            }
    ...
    if (VertexLabelCompat::isIntersectionMode()) {
        ...
        ensureIntersectionMark(data_vertices_num);
        for (ui i = 0; i < query_graph->getVerticesCount(); ++i) {
            ui degree = query_graph->getVertexDegree(i);
            const ui epoch = nextIntersectionEpoch();
            const auto& qtypes = (*VertexLabelCompat::getQueryVertexTypes())[i];
            for (LabelID t : qtypes) {
                if (t >= inv->size()) {
                    continue;
                }
                for (VertexID dv : (*inv)[t]) {
                    if (dv >= data_vertices_num) {
                        continue;
                    }
                    if (g_intersection_visit_mark[dv] == epoch) {
                        continue;
                    }
                    g_intersection_visit_mark[dv] = epoch;
                    if (data_graph->getVertexDegree(dv) >= degree) {
                        candidates[i][candidates_count[i]++] = dv;
                    }
                }
    ...
    for (ui i = 1; i < query_vertices_count; ++i) {
        ...
        for (ui j = 0; j < frontiers_count; ++j) {
            ...
            for (ui k = 0; k < nbrs_cnt; ++k) {
                ...
                if (VertexLabelCompat::vertexMatches(query_graph, u, data_graph, v) &&
                    data_graph->getVertexDegree(v) >= u_degree) {

                    
#if OPTIMIZED_VLABELED_GRAPH == 1
                    bool accept = VertexLabelCompat::isIntersectionMode() || VertexLabelCompat::isIgnoreVertexLabels();
                    if (!accept) {
...
FilterVertices::VEQPruneCandidates(const Graph *data_graph, const Graph *query_graph, VertexID u,
    ...
    for (ui i = 0; i < candidates_count[u]; i++) {
        ...
        if (!VertexLabelCompat::isIntersectionMode() && !VertexLabelCompat::isIgnoreVertexLabels()) {
#ifdef ELABELED_GRAPH
        ...
        for (ui j = 0; j < vnbrs_num; j++) { 
            ...
            ui lcnt = 0;
            const LabelID* labels = data_graph->getEdgeLabelList(v, j, lcnt);
            for (ui k = 0; k < unbrs_num; k++) {
                VertexID unbr = unbrs[k];
                auto iter = D[unbr].find(vc);
                if (iter != D[unbr].end()) {
                    for (ui li = 0; li < lcnt; ++li) {
                        LabelID elabel = labels[li];
...
void FilterVertices::allocateBuffer(const Graph *data_graph, const Graph *query_graph, ui **&candidates,
                                    ui *&candidates_count) {
    ui query_vertices_num = query_graph->getVerticesCount();
    const ui candidates_max_num = VertexLabelCompat::maxCandidateBufferSize(data_graph);
...
FilterVertices::computeCandidateWithNLF(const Graph *data_graph, const Graph *query_graph, VertexID query_vertex,
    ...
    if (VertexLabelCompat::isIgnoreVertexLabels()) {
        const ui dn = data_graph->getVerticesCount();
        for (VertexID data_vertex = 0; data_vertex < dn; ++data_vertex) {
            if (data_graph->getVertexDegree(data_vertex) >= degree) {
                if (buffer != NULL) {
    ...
    if (VertexLabelCompat::isIntersectionMode()) {
        ...
        ensureIntersectionMark(dn);
        const ui epoch = nextIntersectionEpoch();
        const auto* qtptr = VertexLabelCompat::getQueryVertexTypes();
        if (qtptr == nullptr || query_vertex >= qtptr->size()) {
            return;
        }
        const auto& qtypes = (*qtptr)[query_vertex];
        for (LabelID t : qtypes) {
            if (t >= inv->size()) {
                continue;
            }
            for (VertexID data_vertex : (*inv)[t]) {
                if (data_vertex >= dn) {
                    continue;
                }
                if (g_intersection_visit_mark[data_vertex] == epoch) {
                    continue;
                }
                g_intersection_visit_mark[data_vertex] = epoch;
                if (data_graph->getVertexDegree(data_vertex) >= degree) {
                    if (buffer != NULL) {
                        buffer[count] = data_vertex;
                    }
...
void FilterVertices::computeCandidateWithLDF(const Graph *data_graph, const Graph *query_graph, VertexID query_vertex,
    ...
    if (VertexLabelCompat::isIntersectionMode()) {
        ...
        for (LabelID t : qtypes) {
            ...
            for (VertexID v : (*inv)[t]) {
                if (v >= dn) {
                    continue;
                }
                if (g_intersection_visit_mark[v] == epoch) {
                    continue;
                }
                g_intersection_visit_mark[v] = epoch;
                if (data_graph->getVertexDegree(v) >= degree) {
                    if (buffer == NULL) {
                        count += 1;
                    } else {
    ...
    for (ui i = 0; i < pivot_vertices_count; ++i) {
        ...
        for (ui j = 0; j < candidates_count[pivot_vertex]; ++j) {
            ...
            for (ui k = 0; k < v_nbrs_count; ++k) {
                ...
                if (flag[v_nbr] == count && VertexLabelCompat::vertexMatches(query_graph, query_vertex, data_graph, v_nbr) &&
...
#if OPTIMIZED_VLABELED_GRAPH == 1
            bool is_valid = VertexLabelCompat::isIntersectionMode() || VertexLabelCompat::isIgnoreVertexLabels();
            if (!is_valid) {
                auto v_nlf = data_graph->getVertexNLF(v);
                if (v_nlf->size() >= u_nlf->size()) {
                    is_valid = true;
...
FilterVertices::pruneCandidates(const Graph *data_graph, const Graph *query_graph, VertexID query_vertex,
    ...
    for (ui i = 0; i < pivot_vertices_count; ++i) {
        VertexID pivot_vertex = pivot_vertices[i];
        ui qcnt = 0;
        const LabelID* qlabels = nullptr;
        {
            
            ui q_nbr_cnt = 0;
            const VertexID* q_nbrs = query_graph->getVertexNeighbors(query_vertex, q_nbr_cnt);
            ui nbr_local_idx = q_nbr_cnt;
            for (ui pos = 0; pos < q_nbr_cnt; ++pos) {
                if (q_nbrs[pos] == pivot_vertex) {
                    nbr_local_idx = pos;
                    break;
                }
            }
            if (nbr_local_idx < q_nbr_cnt) {
                qlabels = query_graph->getEdgeLabelList(query_vertex, nbr_local_idx, qcnt);
            }
        }
        ...
        for (ui j = 0; j < candidates_count[pivot_vertex]; ++j) {
            ...
            for (ui k = 0; k < v_nbrs_count; ++k) {
                ...
                bool label_ok = (qcnt == 0);
                for (ui qi = 0; qi < qcnt; ++qi) {
                    if (data_graph->edgeHasLabelAt(v, k, qlabels[qi])) { label_ok = true; break; }
                }

                if (flag[v_nbr] == count
                    && VertexLabelCompat::vertexMatches(query_graph, query_vertex, data_graph, v_nbr)
                    && v_nbr_degree >= query_vertex_degree
                    && label_ok) {
                    flag[v_nbr] += 1;
