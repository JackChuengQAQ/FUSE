...
public:
    struct TopKResult {
        double score;
        std::vector<ui> mapping;
        std::vector<VertexID> edge_sources;
        std::vector<VertexID> edge_targets;
        std::vector<ui> edge_relation_ids;
    };
    ...
    static void configureTopKScoring(const EmbeddingDiGraph* query_embedding_graph,
                                     const EmbeddingDiGraph* data_embedding_graph,
                                     size_t topk_limit);
    static std::vector<TopKResult> getTopKResults();
    static void clearTopKResults();

    
    static void setEnumerationTimeLimitFromNowSeconds(std::uint64_t seconds);
    static void clearEnumerationTimeLimit();
    static bool tookEnumerationTimeLimit();
...
private:
    ...
    static const EmbeddingDiGraph* query_embedding_graph_;
    static const EmbeddingDiGraph* data_embedding_graph_;
    static size_t topk_limit_;
    static std::vector<TopKResult> topk_results_;
    static bool enumeration_time_limit_active_;
    static std::chrono::steady_clock::time_point enumeration_deadline_;
    static bool enumeration_timed_out_;
    static std::uint64_t enumeration_lftj_tick_;
    static void updateTopKResults(const ui* embedding, ui query_vertices_num);
    static double getCurrentTopKUpperBound();
