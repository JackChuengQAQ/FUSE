...
namespace {
thread_local std::vector<ui> g_vf3_intersection_visit_mark;
thread_local ui g_vf3_intersection_epoch = 1;

static ui nextVf3IntersectionEpoch() {
    g_vf3_intersection_epoch += 1;
    if (g_vf3_intersection_epoch == 0) {
        std::fill(g_vf3_intersection_visit_mark.begin(), g_vf3_intersection_visit_mark.end(), 0);
        g_vf3_intersection_epoch = 1;
    }
    return g_vf3_intersection_epoch;
}

static void ensureVf3IntersectionMark(ui data_vertices_num) {
    if (g_vf3_intersection_visit_mark.size() < data_vertices_num) {
        g_vf3_intersection_visit_mark.resize(data_vertices_num, 0);
    }
}
}  
...
const EmbeddingDiGraph* EvaluateQuery::query_embedding_graph_ = nullptr;
const EmbeddingDiGraph* EvaluateQuery::data_embedding_graph_ = nullptr;
size_t EvaluateQuery::topk_limit_ = 0;
std::vector<EvaluateQuery::TopKResult> EvaluateQuery::topk_results_;
bool EvaluateQuery::enumeration_time_limit_active_ = false;
std::chrono::steady_clock::time_point EvaluateQuery::enumeration_deadline_{};
bool EvaluateQuery::enumeration_timed_out_ = false;
std::uint64_t EvaluateQuery::enumeration_lftj_tick_ = 0;
...
void EvaluateQuery::configureTopKScoring(const EmbeddingDiGraph* query_embedding_graph,
                                         const EmbeddingDiGraph* data_embedding_graph,
                                         size_t topk_limit) {
    query_embedding_graph_ = query_embedding_graph;
    data_embedding_graph_ = data_embedding_graph;
    topk_limit_ = topk_limit;
    topk_results_.clear();
}

void EvaluateQuery::clearTopKResults() {
    topk_results_.clear();
}

void EvaluateQuery::setEnumerationTimeLimitFromNowSeconds(std::uint64_t seconds) {
    enumeration_timed_out_ = false;
    enumeration_lftj_tick_ = 0;
    if (seconds == 0) {
        enumeration_time_limit_active_ = false;
        return;
    }
    enumeration_time_limit_active_ = true;
    enumeration_deadline_ = std::chrono::steady_clock::now() + std::chrono::seconds(static_cast<long long>(seconds));
}

void EvaluateQuery::clearEnumerationTimeLimit() {
    enumeration_time_limit_active_ = false;
    enumeration_timed_out_ = false;
}

bool EvaluateQuery::tookEnumerationTimeLimit() {
    return enumeration_timed_out_;
}

std::vector<EvaluateQuery::TopKResult> EvaluateQuery::getTopKResults() {
    auto results = topk_results_;
    std::sort(results.begin(), results.end(),
              [](const EvaluateQuery::TopKResult& lhs, const EvaluateQuery::TopKResult& rhs) {
                  return lhs.score < rhs.score;
              });
    std::vector<EvaluateQuery::TopKResult> unique;
    unique.reserve(results.size());
    for (const auto& r : results) {
        bool dup = false;
        for (const auto& u : unique) {
            if (u.mapping.size() == r.mapping.size() &&
                std::equal(u.mapping.begin(), u.mapping.end(), r.mapping.begin())) {
                dup = true;
                break;
            }
        }
...
double EvaluateQuery::getCurrentTopKUpperBound() {
    if (topk_limit_ == 0 || topk_results_.size() < topk_limit_) {
        return std::numeric_limits<double>::infinity();
    }

    double worst_score = topk_results_[0].score;
    for (size_t i = 1; i < topk_results_.size(); ++i) {
        if (topk_results_[i].score > worst_score) {
            worst_score = topk_results_[i].score;
        }
    }
    return worst_score;
}

void EvaluateQuery::updateTopKResults(const ui* embedding, ui query_vertices_num) {
    if (topk_limit_ == 0 || query_embedding_graph_ == nullptr || data_embedding_graph_ == nullptr) {
        return;
    }

    ui query_vertices = query_embedding_graph_->getVerticesCount();
    ui data_vertices = data_embedding_graph_->getVerticesCount();
    if (query_vertices_num != query_vertices) {
        return;
    }

    ui qdim = query_embedding_graph_->getVertexEmbeddingDim();
    ui ddim = data_embedding_graph_->getVertexEmbeddingDim();
    ui dim = std::min(qdim, ddim);

    std::vector<ui> mapping(query_vertices_num);
    for (ui u = 0; u < query_vertices_num; ++u) {
        mapping[u] = embedding[u];
    }
    ...
    if (dim > 0) {
        for (ui u = 0; u < query_vertices_num; ++u) {
            ...
            const float* qemb = query_embedding_graph_->getVertexEmbedding(u);
            const float* demb = data_embedding_graph_->getVertexEmbedding(v);
            double vertex_dist2 = 0.0;
            for (ui i = 0; i < dim; ++i) {
                double diff = static_cast<double>(qemb[i]) - static_cast<double>(demb[i]);
                vertex_dist2 += diff * diff;
            }
            total_score += std::sqrt(vertex_dist2);
        }
    } else {
        
        for (ui u = 0; u < query_vertices_num; ++u) {
            if (embedding[u] >= data_vertices) {
                return;
            }
        }
    }

    const auto mapping_is_duplicate = [&](const std::vector<ui>& m) {
        for (const auto& ex : topk_results_) {
            if (ex.mapping.size() == m.size() && std::equal(ex.mapping.begin(), ex.mapping.end(), m.begin())) {
                return true;
            }
        }
        return false;
    ...
    auto push_result = [&](EvaluateQuery::TopKResult&& result) {
        if (topk_limit_ == 0) {
            return;
        }
        if (mapping_is_duplicate(result.mapping)) {
            return;
        }

        if (topk_results_.size() < topk_limit_) {
            topk_results_.emplace_back(std::move(result));
            return;
        }

        size_t worst_idx = 0;
        for (size_t i = 1; i < topk_results_.size(); ++i) {
            if (topk_results_[i].score > topk_results_[worst_idx].score) {
                worst_idx = i;
            }
        }

        if (result.score < topk_results_[worst_idx].score) {
            topk_results_[worst_idx] = std::move(result);
        }
    };

    EvaluateQuery::TopKResult result;
    result.score = (dim > 0) ? total_score : 0.0;
    result.mapping = std::move(mapping);
    push_result(std::move(result));
}

namespace {
inline bool shouldPrintEmbeddings() {
    const char* env = std::getenv("NUGM_PRINT_EMBEDDINGS");
    return env != nullptr && env[0] != 0 && !(env[0] == '0' && env[1] == 0);
}

inline size_t getEmbeddingPrintLimit() {
    const char* env = std::getenv("NUGM_PRINT_EMBEDDINGS_MAX");
    if (env == nullptr || env[0] == 0) return 20;
    char* end_ptr = nullptr;
    unsigned long long parsed = std::strtoull(env, &end_ptr, 10);
    if (end_ptr == env || (end_ptr != nullptr && *end_ptr != 0) || parsed == 0) return 20;
    return static_cast<size_t>(parsed);
}

inline bool shouldPrintProgress() {
    
    
    const char* env = std::getenv("NUGM_PRINT_PROGRESS");
    return env != nullptr && env[0] == '1' && env[1] == 0;
}

inline bool shouldEnableTopKPruning() {
    
    const char* env = std::getenv("NUGM_TOPK_PRUNE");
    return !(env != nullptr && env[0] == '0' && env[1] == 0);
}

inline void printEmbeddingResult(const Graph* query_graph, const Graph* data_graph, const ui* order,
                                 const ui* embedding, ui query_vertices_num, size_t embedding_id) {
    std::cout << "Embedding " << embedding_id << ": ";
    for (ui i = 0; i < query_vertices_num; ++i) {
        ui u = order[i];
        std::cout << "q" << u << "->" << embedding[u];
        if (i + 1 != query_vertices_num) std::cout << ", ";
    }
    std::cout << std::endl;

    std::cout << "  Edges: ";
    bool first_edge = true;
    for (ui begin = 0; begin < query_vertices_num; ++begin) {
        ui nbr_count = 0;
        const ui* nbrs = query_graph->getVertexNeighbors(begin, nbr_count);
        for (ui j = 0; j < nbr_count; ++j) {
            ...
            LabelID query_elabel = query_graph->getEdgeLabel(begin, end, false);
            LabelID data_elabel = data_graph->getEdgeLabel(embedding[begin], embedding[end], false);
#endif
            if (!first_edge) {
                std::cout << " | ";
            }
            ...
            std::cout << "q" << begin << "-q" << end << " => "
                      << "d" << embedding[begin] << "-d" << embedding[end];
#ifdef ELABELED_GRAPH
            std::cout << " [qrel=" << query_elabel << ", drel=" << data_elabel << "]";
#endif
        }
    }
    std::cout << std::endl;
...
void EvaluateQuery::generateValidCandidateIndex(const Graph *data_graph, ui depth, ui *embedding, ui *idx_embedding,
    ...
    if (bn_cnt[depth] == 0) {
        ...
        std::vector<ui> bn_order;
        bn_order.reserve(bn_cnt[depth]);
        for (ui j = 0; j < bn_cnt[depth]; ++j) {
            bn_order.push_back(j);
        }

        std::sort(bn_order.begin(), bn_order.end(), [&](ui lhs, ui rhs) {
            VertexID lhs_bn = bn[depth][lhs];
            VertexID rhs_bn = bn[depth][rhs];
            return data_graph->getVertexDegree(embedding[lhs_bn]) < data_graph->getVertexDegree(embedding[rhs_bn]);
        });
        ...
        for (ui i = 0; i < count; ++i) {
            ...
            if (!visited_vertices[temp_v]) {
                ...
                for (ui ordered_bn_idx : bn_order) {
                    VertexID u_bn = bn[depth][ordered_bn_idx];
...
EvaluateQuery::LFTJ(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates,
    ...
    const size_t effective_output_limit =
        (topk_limit_ > 0) ? std::numeric_limits<size_t>::max() : output_limit_num;

    const bool print_embeddings = shouldPrintEmbeddings();
    const size_t print_limit = getEmbeddingPrintLimit();
    const bool print_progress = shouldPrintProgress();
    size_t printed_embeddings = 0;
    int cur_depth = 0;
    ui max_depth = query_graph->getVerticesCount();
    VertexID start_vertex = order[0];
    const ui prune_dim =
        (topk_limit_ > 0 && query_embedding_graph_ != nullptr && data_embedding_graph_ != nullptr &&
         query_embedding_graph_->getVerticesCount() == max_depth)
            ? std::min(query_embedding_graph_->getVertexEmbeddingDim(), data_embedding_graph_->getVertexEmbeddingDim())
            : 0;
    const bool enable_topk_prune = shouldEnableTopKPruning() && (prune_dim > 0);
    
    
    const bool enable_failing_set_runtime = !enable_topk_prune;
    const double topk_prune_eps = 1e-12;
    std::vector<double> partial_dist_sum(max_depth, 0.0);
    ...
    while (true) {
        while (idx[cur_depth] < idx_count[cur_depth]) {
            if (enumeration_time_limit_active_) {
                if ((++enumeration_lftj_tick_ & 8191ull) == 0ull) {
                    if (std::chrono::steady_clock::now() >= enumeration_deadline_) {
                        enumeration_timed_out_ = true;
                        goto EXIT;
                    }
                }
            }
            ...
            double new_partial_dist_sum = 0.0;
            if (enable_topk_prune && query_embedding_graph_ != nullptr && data_embedding_graph_ != nullptr &&
                u < query_embedding_graph_->getVerticesCount() &&
                v < data_embedding_graph_->getVerticesCount()) {
                const float* qemb = query_embedding_graph_->getVertexEmbedding(u);
                const float* demb = data_embedding_graph_->getVertexEmbedding(v);
                const ui safe_dim = std::min(
                    prune_dim, std::min(query_embedding_graph_->getVertexEmbeddingDim(),
                                        data_embedding_graph_->getVertexEmbeddingDim()));
                double vertex_dist2 = 0.0;
                for (ui i = 0; i < safe_dim; ++i) {
                    double diff = static_cast<double>(qemb[i]) - static_cast<double>(demb[i]);
                    vertex_dist2 += diff * diff;
                }
                const double vertex_l2 = std::sqrt(vertex_dist2);

                double prefix = (cur_depth == 0) ? 0.0 : partial_dist_sum[cur_depth - 1];

                new_partial_dist_sum = prefix + vertex_l2;
                
                if (topk_results_.size() >= topk_limit_) {
                    double ub = getCurrentTopKUpperBound();
                    if (ub < std::numeric_limits<double>::infinity() &&
                        new_partial_dist_sum > ub + topk_prune_eps) {
                        idx[cur_depth] += 1;
                        continue;
                    }
                }
            } else if (enable_topk_prune) {
                new_partial_dist_sum =
                    (cur_depth == 0) ? 0.0 : partial_dist_sum[cur_depth - 1];
            }

            embedding[u] = v;
            idx_embedding[u] = valid_idx;
            if (enable_topk_prune) {
                partial_dist_sum[cur_depth] = new_partial_dist_sum;
            }
            visited_vertices[v] = true;
            idx[cur_depth] += 1;
            ...
            if (cur_depth == max_depth - 1) {
                ...
                if (print_embeddings && printed_embeddings < print_limit) {
                    printEmbeddingResult(query_graph, data_graph, order, embedding, max_depth, embedding_cnt + 1);
                    printed_embeddings += 1;
                }
                updateTopKResults(embedding, max_depth);
                embedding_cnt += 1;
                if (print_progress && embedding_cnt % 500000 == 0) {
                    std::cout << "Progress: " << embedding_cnt << " embeddings enumerated." << std::endl;
                }
                visited_vertices[v] = false;
...
#endif
                if (embedding_cnt >= effective_output_limit) {
                    goto EXIT;
                }
            } else {
                call_count += 1;
...
void EvaluateQuery::generateValidCandidateIndex(ui depth, ui *idx_embedding, ui *idx_count, ui **valid_candidate_index,
    ...
    for (ui i = 1; i < bn_cnt[depth]; ++i) {
        ...
        std::memcpy(valid_candidate_index[depth], temp_buffer, temp_count * sizeof(ui));
...
size_t EvaluateQuery::exploreQuickSIStyle(const Graph *data_graph, const Graph *query_graph, ui **candidates,
    ...
    ui max_candidate_count = candidates_count[start_vertex];
    for (ui i = 1; i < max_depth; ++i) {
        if (candidates_count[i] > max_candidate_count) {
            max_candidate_count = candidates_count[i];
        }
    }
    if (max_candidate_count == 0) {
        max_candidate_count = VertexLabelCompat::maxCandidateBufferSize(data_graph);
    }
    for (ui i = 0; i < max_depth; ++i) {
        valid_candidate[i] = new VertexID[max_candidate_count];
    }
...
#endif
        if (!visited_vertices[v] && VertexLabelCompat::vertexMatches(query_graph, u, data_graph, v) &&
...
EvaluateQuery::generateValidCandidateIndex(VertexID u, ui *idx_embedding, ui *idx_count, ui *&valid_candidate_index,
    ...
    for (ui i = 1; i < bn_cnt; ++i) {
        ...
        std::memcpy(valid_candidate_index, temp_buffer, temp_count * sizeof(ui));
        valid_candidates_count = temp_count;
    }
...
bool EvaluateQuery::exploreVF3Backtrack(const Graph *data_graph, const Graph *query_graph,
    ...
    if (depth == 0) {
        ...
        for (int i = 0; i < (int)idx_count[depth]; ++i) {
            std::vector<ui> embedding_local(max_depth+1);
            std::vector<ui> idx_local(max_depth+1);
            std::vector<ui> idx_count_local(max_depth+1);
            
            std::unique_ptr<bool[]> visited_vertices_local(new bool[data_graph->getVerticesCount()]);
            for (ui d = 0; d < max_depth+1; ++d) {
                embedding_local[d] = embedding[d];
                idx_local[d] = idx[d];
                idx_count_local[d] = idx_count[d];
            }
            for (ui v = 0; v < data_graph->getVerticesCount(); ++v) {
                visited_vertices_local[v] = visited_vertices[v];
            }
            embedding_local[order[depth]] = valid_candidates[depth][i];
            visited_vertices_local[valid_candidates[depth][i]] = true;
            size_t call_count_local = 0;
            ui embedding_cnt_local = 0;
            if (isFeasibility(data_graph, query_graph, depth, order[depth], valid_candidates[depth][i],
                            embedding_local.data(), order, query_feasibility, query_feasibility_count,
                            data_feasibility, data_feasibility_count)) {
                exploreVF3Backtrack(data_graph, query_graph, order, pivot, output_limit_num, call_count_local, embedding_cnt_local,
                        depth + 1, max_depth, idx_local.data(), idx_count_local.data(), embedding_local.data(), valid_candidates, visited_vertices_local.get(),
                        query_feasibility, query_feasibility_count, data_feasibility, data_feasibility_count);
                #pragma omp atomic
                embedding_cnt += embedding_cnt_local;
                #pragma omp atomic
                call_count += call_count_local;
            }
        }
        return embedding_cnt >= output_limit_num;
    }
    ...
    if (pivot[depth] == (ui)-1) {
        if (VertexLabelCompat::isIntersectionMode()) {
            const auto* qtptr = VertexLabelCompat::getQueryVertexTypes();
            const auto* inv = VertexLabelCompat::getDataInvertedIndex();
            ui dn = data_graph->getVerticesCount();
            ensureVf3IntersectionMark(dn);
            const ui epoch = nextVf3IntersectionEpoch();
            if (qtptr != nullptr && inv != nullptr && u < qtptr->size()) {
                for (LabelID t : (*qtptr)[u]) {
                    if (t >= inv->size()) {
                        continue;
                    }
                    for (VertexID cand : (*inv)[t]) {
                        if (cand >= dn) {
                            continue;
                        }
                        if (!visited_vertices[cand] && g_vf3_intersection_visit_mark[cand] != epoch) {
                            g_vf3_intersection_visit_mark[cand] = epoch;
                            valid_candidates[depth][idx_count[depth]++] = cand;
                        }
                    }
                }
            }
        } else if (VertexLabelCompat::isIgnoreVertexLabels()) {
            ui dn = data_graph->getVerticesCount();
            ui u_degree = query_graph->getVertexDegree(u);
            for (VertexID cand = 0; cand < dn; ++cand) {
                if (!visited_vertices[cand] && data_graph->getVertexDegree(cand) >= u_degree) {
                    valid_candidates[depth][idx_count[depth]++] = cand;
                }
            }
        } else {
        ...
        for (ui i = 0; i < candidate_count; i++) {
            if (!visited_vertices[candidates[i]] &&
                VertexLabelCompat::vertexMatches(query_graph, u, data_graph, candidates[i])) {
                valid_candidates[depth][idx_count[depth]++] = candidates[i];
            }
        }
    }
...
#ifdef ELABELED_GRAPH
            ui qcnt = 0;
            const LabelID* qlabels = query_graph->getEdgeLabelList(cur_u, i, qcnt);
            bool ok = false;
            for (ui qi = 0; qi < qcnt; ++qi) {
                if (data_graph->checkEdgeExistence(cur_v, embedding[unbr], qlabels[qi])) {
                    ok = true;
                    break;
                }
            }
    ...
    for (ui i = 0; i < candidates_count[u]; i++) {
        ...
        for (ui j = 0; j < unbrs_count; j++) {
            if (visited_u[unbrs[j]] == true
                ...
                ui qcnt = 0;
                const LabelID* qlabels = query_graph->getEdgeLabelList(u, j, qcnt);
                bool ok = false;
                for (ui qi = 0; qi < qcnt; ++qi) {
                    if (data_graph->checkEdgeExistence(v, embedding[unbrs[j]], qlabels[qi])) { ok = true; break; }
                }
                if (!ok) {
                    flag = false;
                    break;
            ...
            for (k1 = 0; k1 < unbrs_num; k1++) {
                ...
                for (ui k2 = 0; k2 < u2_nbrs_num; k2++) {
                    if (unbrs[k1] == u2_nbrs[k2]) {
                        ...
                        ui c1 = 0, c2 = 0;
                        const LabelID* l1 = query_graph->getEdgeLabelList(i, k1, c1);
                        const LabelID* l2 = query_graph->getEdgeLabelList(j, k2, c2);
                        
                        ui p1 = 0, p2 = 0;
                        while (p1 < c1 && p2 < c2) {
                            if (l1[p1] == l2[p2]) { flag = true; break; }
                            if (l1[p1] < l2[p2]) ++p1;
                            else ++p2;
                        }
                        break;
