...
public:
    ...
    std::unordered_map<uint32_t, std::pair<uint32_t, uint32_t>>* trie_;
    uint32_t cardinality_;
    uint32_t max_degree_;
    uint32_t* children_;
public:
    hash_relation() {
        trie_ = new std::unordered_map<uint32_t, std::pair<uint32_t, uint32_t>>();
        cardinality_ = 0;
        max_degree_ = 0;
        children_ = nullptr;
    }
    ...
    bool contains(uint32_t key) {
        return trie_->find(key) != trie_->end();
    }
