...
protected:
    ...
    virtual void execute_operator(std::vector<flat_relation *> &relations, size_t& call_count) = 0;
...
public:
    ...
    uint64_t execute(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit, size_t& call_count) {
        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator(relations, call_count);
...
protected:
    ...
    void execute_operator(std::vector<flat_relation *> &relations, size_t& call_count) override;
